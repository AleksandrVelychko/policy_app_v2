...
export default function AdminDashboard({ employees, policies, latestPolicyId, recentPolicyId, onDeletePolicy }) {
  ...
  function exportCsv() {
    setExporting(true);
    try {
      const header = ['Employee', ...policies.map(p => p.title.replace(/,/g,' '))];
      const rows = employees.map(emp => [
        emp.name,
        ...policies.map(p => emp.acknowledged.includes(p.id) ? 'Yes' : 'No')
      ]);
      ...
      a.href = url;
      a.download = 'policy_acknowledgements.csv';
    ...
    <div className="max-w-5xl mx-auto bg-white p-6 rounded shadow mt-8">
      <div className="flex items-start justify-between gap-4 mb-2">
        <h2 className="text-xl font-bold flex items-center gap-2">
          Admin Dashboard <span className="text-sm font-normal text-gray-500">(Acknowledgements)</span>
        </h2>
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={exportCsv}
            disabled={exporting}
            className={`text-xs px-3 py-1 rounded border bg-gray-50 hover:bg-gray-100 ${exporting ? 'opacity-50 cursor-wait' : ''}`}
            title="Export acknowledgement matrix as CSV"
            type="button"
            ...
            <tr>
              ...
              {policies.map((p, idx) => {
                ...
                return (
                  <th
                    key={p.id}
                    className={`border-b p-2 font-medium whitespace-nowrap relative ${isLatest ? 'bg-blue-50' : ''} ${isRecent ? 'ring-1 ring-green-400' : ''}`}
                    title={isLatest ? 'Latest policy version' : isRecent ? 'Recently added' : undefined}
                  >
                    <div className="flex items-center gap-1">
                      <span className="text-gray-500 text-[10px] font-mono bg-gray-100 px-1 rounded">{idx+1}</span>
                      <span>{p.title}</span>
          ...
          <tbody>
            {filteredEmployees.map(emp => (
              <tr key={emp.id} className="even:bg-gray-50">
                <td className="border-b p-2 font-medium whitespace-nowrap">{emp.name}</td>
                {policies.map(p => {
